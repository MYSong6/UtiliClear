...
/* The ECC encode function */
/* It encodes a messsage (i.e., 'm') to a codeword (i.e., 'c')*/
...
/* This function encode 'x 'to 2040 bit-length of codeword,     */
/* appends '0's to 'data' to attain length of 2040,				*/
/* executes the XOR operation on the codeword and the 'data'.   */
string Mask_Code(vector<char> data, string x) {
	...
	for (int i = 0; i< x_code_str.size(); i++) {
...
/* Helper function to generate challenge codeword with insignificant bits and perturbation embedded (correspond to the value z in the paper) */
/* The floder 'inputDir1' only contains files of grouped insignificant bits for the fingerprinted database. (each column corresponds to one file)															*/
/* The floder 'inputDir2' only contains files (storing the value u) used during the locking phase. (each column corresponds to one file)																	*/
/* After the execution, the output floder 'outputDir1' will contain the results (corresponding to the value z=Perm(x \oplus u, \delta) in the paper) of each column (each column corresponds to one file) */
/* After the execution, the output floder 'outputDir2' will contain the randomly selected messages (corresponding to the value m in the paper) of each column (each column corresponds to one file)       */
void processFiles(const std::string& inputDirectory1, const std::string& inputDirectory2, string outputDirectory1, string outputDirectory2,int seed) {
	...
	for (size_t i = 0; i < files1.size(); ++i) {
		ifstream file1(inputDirectory1 + "\\" + files1[i]);  // Open one file in inputDirectory1 storing (insignificant bits)
		ifstream file2(inputDirectory2 + "\\" + files2[i]);  // Open one file in inputDirectory2 storing (random bit-strings u)
		ofstream file3(outputDirectory1 + "\\" + files1[i]); // Open one file in outputDirectory1 to store (z = perturbated bit-string XOR the codeword of a random message)	
		ofstream file4(outputDirectory2 + "\\" + files1[i]); // Open one file in outputDirectory2 to store (random messgaes)
		...
		while (getline(file1, line1)&&getline(file2,line2)) { // Read one line from file1 and read one line from file2
			vector<char> res;
			for (int j = 0; j < line1.size(); j++) {		  // line1 XOR line2
				if (line1[j] == line2[j]) {
					res.push_back('0');
				}
				else {
			...
			shuffle(res.begin(), res.end(), eng);				// Permutation
			string msg = generate_random_binary_string(1992);	// Randomly select a message with length of 1992.
			string masked_code = Mask_Code(res, msg);			// Encode msg to 2040 bit-length of codeword, append '0's to res to attain length of 2040, execute the XOR operation on the codeword and the appended result.


			file3 << masked_code << endl;						// Write the output result (z) to file in 'outputDirectory1'
			file4 << msg << endl;								// Write the selected message (m) to the file in 'outputDirectory2'
...
/* It inputs two floders 'inputDir1' and 'inputDir2',and output two floders 'outputDir1' and 'outputDir2'																									*/
/* The floder 'inputDir1' only contains files of grouped insignificant bits for the fingerprinted database. (each column corresponds to one file)															*/
/* The floder 'inputDir2' only contains files of perturbation used during the locking phase. (each column corresponds to one file)																			*/
...
/* Before executing, make sure that the two output floders are empty.																																		*/
/* After the execution, the output floder 'outputDir1' will contain the results (corresponding to the value z=Perm(x \oplus u, \delta) in the paper) of each column (each column corresponds to one file) */
/* After the execution, the output floder 'outputDir2' will contain the randomly selected messages (corresponding to the value m in the paper) of each column (each column corresponds to one file)       */

int main(int argc, const char* argv[]) {
	
	const int seed = 211;																// The seed of permutation function
	string inputDir1 = "C:/Users/62555/Desktop/Paper8Data/Insign_group";			    // Replace with your path storing the files containint insignificant bits of each column (fingerprinted database)
	string inputDir2 = "C:/Users/62555/Desktop/Paper8Data/Insignificant_u";				// Replace with your path storing the perturbations of each column	(used in the locking phase)					
	string outputDir1 = "C:/Users/62555/Desktop/Paper8Data/Insignificant_z";			// Replace with your path to store the results (z) of each column   (each item with bit-length 2040)
	string outputDir2 = "C:/Users/62555/Desktop/Paper8Data/Insignificant_m";			// Replace with your path to store the randomly selected message (m) of each column	(each item with bit-length 1992)
	processFiles(inputDir1, inputDir2, outputDir1, outputDir2, seed);
