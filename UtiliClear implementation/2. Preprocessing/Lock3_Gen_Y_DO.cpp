...
/* Helper function to decrypt pertrubed GM ciphertext                                                                               */
/* The floder 'inputDirectory' contains files of pertrubed GM ciphertext (each column corresponds to a file)                       */
/* The function reads each file in a folder (i.e., inputDirectory) and then computes its corresponding plaintext y                  */
/* The plaintext for the messages from the same file are stored in an identical file using the same name as the original input file */
/* All the output files are stored in an identical foloder (i.e., outputDirectory).                                                 */
void Generate_Y(const std::string& inputDirectory, const std::string& outputDirectory) {
	...
	do {
		if (!(findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
			...
			while (getline(inFile, line1)) {
				...
				input_v.push_back(line1);
				string result = DecGM(p,q, R,n, input_v);
				outFile << result;
			}
		}
...
int main(int argc, const char* argv[]) {
	...
	string inputDir = "C:/Users/62555/Desktop/Paper8Data/Insignificant_cy";   // Replace with your path storing the Cy of each column
	string outputDir = "C:/Users/62555/Desktop/Paper8Data/Insignificant_y";   // Replace with your path storing the y reslts of each column

	Generate_Y(inputDir, outputDir);
