...
int main(int argc, const char * argv[]) {
    ...
    const int NumCol=xx;
    const int NumRow = xxx;
    const int Num_group = xxx;
    int total_num = Num_InsignBits*NumRow;
    fstream read_code1,read_code2;
    string code_path1 = "xxxx";
    string code_path2 = "xxxx";
    read_code1.open(code_path1);
    read_code2.open(code_path2);
    
    
    
    for(int i=0;i<NumCol;i++){
        
        string code,code_proof;
        getline(read_code1,code);
        getline(read_code2,code_proof);
        
        int diff_num = 0;
       
       
        for(int j=0;j<total_num ;j++){
            if(code[j]!=code_proof[j]){
                diff_num ++;
            }
        }
        if(diff_num<Num_group*Num_InsignBits){
            cout<<"col"<<i+1<<": Success~~"<<endl;
        }
