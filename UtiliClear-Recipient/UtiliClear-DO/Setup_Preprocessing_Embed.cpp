...
string Fuzzy_Commit_Proof(unsigned char c[249], string x) {
	unsigned char xx[249], x_coded[255];
	int index = 0;
	int orNum = x.size();
	for (int i = orNum; i < 2040; i++) {
		if (i % 2 == 0) {
			x += '0';
		}
		else {
	...
	binaryStringToUnsignedCharArray(x, x_coded);
	int fl = 0;
	rsdecode(x_coded, xx, &fl);
	string xx_str = unsignedCharArrayToBinaryString(xx, 249);
	string cc_str = unsignedCharArrayToBinaryString(c, 249);
	string s_str;

	for (int i = 0; i<xx_str.size(); i++) {
		if (xx_str[i] != cc_str[i]) {
			s_str += '1';
		}
		else {
			s_str += '0';
...
void Fuzzy_Commit_Verify(string s_str, string x1) {
	unsigned char x1_coded[255], xx1[249], c[249];
	int orNum = x1.size();
	for (int i = orNum; i < 2040; i++) {
		if (i % 2 == 0) {
			x1 += '0';
	...
	binaryStringToUnsignedCharArray(x1, x1_coded);
	int fl = 0;
	rsdecode(x1_coded, xx1, &fl);
	string xx1_str = unsignedCharArrayToBinaryString(xx1, 249);

	string c_str;
	for (int i = 0; i < xx1_str.size(); i++) {
		if (xx1_str[i] != s_str[i]) {
			c_str += '1';
		}
		else {
			c_str += '0';
	...
	binaryStringToUnsignedCharArray(c_str, c);

	char * DataChar = new char[249 + 1];
	for (int j = 0; j<249; j++) {
		DataChar[j] = c[j];
	}
	DataChar[249] = '\0';
...
void PC_Group(string DB_name, map<int, vector<string>> &group_Sign, map<int, vector<string>> &group_InSign, string NumGroup_str, int Num_InsignBits, int Num_row, int att_ord, string lamda_str) {
	string select_sql = "SELECT * FROM db_info WHERE DB='" + DB_name + "';";
	DB_Query(select_sql);
	string att = nextRow[2 + att_ord];
	int NumGroup = atoi(NumGroup_str.c_str());
	for (int i = 1; i <= Num_row; i++) {

		string seed_str = lamda_str+ to_string(att_ord)+ to_string(i); //ID+attr+lamda
		...
		//group_f[mod_res].push_back(i);
		string SQL_str = "SELECT " + att + " FROM " + DB_name + " WHERE ID='" + to_string(i) + "';";
		DB_Query(SQL_str);
		string SQL_Res_str = nextRow[0];
		string Sign_str = SQL_Res_str.substr(0, SQL_Res_str.size() - Num_InsignBits);
		string InSign_str = SQL_Res_str.substr(SQL_Res_str.size() - Num_InsignBits, Num_InsignBits);
		group_Sign[mod_res].push_back(Sign_str);
		group_InSign[mod_res].push_back(InSign_str);
	}
...
void Fuzzy_Commit(map<int, vector<string>> group_InSign, vector<vector<string>> &Comm, int Num_Group) {
	...
	for (int i = 0; i < Num_Group; i++) {
		...
		for (int j = 0; j < group_InSign[i].size(); j++) {
			msg_str += group_InSign[i][j];
		}
		if (msg_str.size() == 0) {
			vector<string> c;
			Comm.push_back(c);
...
int main(int argc, const char* argv[]) {
	...
	N_str = nextRow[2];
	g_str = nextRow[3];
	endTime_client = clock();
	Setup_comput_time += (double)(endTime_client - startTime_client) / CLOCKS_PER_SEC;
	...
	string Num_col = nextRow[2]; //最后一列是编号
	endTime_client = clock();
	Setup_comput_time += (double)(endTime_client - startTime_client) / CLOCKS_PER_SEC;//////
	...
	string lamda_str = Info_Receive();
	string Num_MaxModifyBits_str = Info_Receive();
	string Fingerprint = Info_Receive();
	...
	string DB_path = "xxxxx";
	PC_Group_New(DB_path, group_Sign, group_InSign, Num_Group_str, atoi(Num_InsignBits_str.c_str()), atoi(Num_row.c_str()), atoi(Num_col.c_str()) - 1, lamda_str, Grouped);
	
	PC_Commit_New(group_Sign, Comm, d_vec, N_str, g_str, h_str, atoi(Num_Group_str.c_str()), atoi(Num_InsignBits_str.c_str()), atoi(Num_col.c_str())-1);
	...
	for (int i = 1; i < atoi(Num_col.c_str()); i++) {
		string path1 = "xxxxx";
		fstream write_ptr1;
		write_ptr1.open(path1);
		string d_str = d_vec[i - 1];
		write_ptr1 << d_str << endl;
		for (int gid = 0; gid < atoi(Num_Group_str.c_str()); gid++) {
			write_ptr1 <<Comm[i][gid] << endl;
		}
		write_ptr1.close();
	}
	...
	for(int i=1; i < atoi(Num_col.c_str()); i++){
		...
		vector<vector<string>> fuzzy_comm;
		Fuzzy_Commit_New(group_InSign[i], fuzzy_comm, atoi(Num_Group_str.c_str()));
		...
		string path2 = "xxxxx";
		fstream write_ptr2;
		write_ptr2.open(path2);
		for (int gid = 0; gid < atoi(Num_Group_str.c_str()); gid++) {
			for (int j = 0; j < fuzzy_comm[gid].size(); j++) {
				write_ptr1 << fuzzy_comm[gid][j] << endl;
			}
			write_ptr2 << to_string(group_InSign[i][gid].size()) << endl;
		}
		write_ptr1.close();
		write_ptr2.close();
		
	}
	cout << "Proof 写入完成" << endl;


	string output_path = "xxxx";

	
	FingerPrinting(Fingerprint,sk,DB_path, output_path, Grouped, atoi(Num_Group_str.c_str()), atoi(Num_row.c_str()), atoi(Num_col.c_str())-1, atoi(Num_InsignBits_str.c_str()), atoi(Num_MaxModifyBits_str.c_str()));
	
	

	

	fstream write_userInfo;
	string user_information_path = "xxxxx";
	write_userInfo.open(user_information_path,ios::out);
	write_userInfo << Num_InsignBits_str << endl;
	write_userInfo << Num_Group_str << endl;
	write_userInfo << lamda_str << endl;
	write_userInfo << Fingerprint << endl;
	write_userInfo.close();
