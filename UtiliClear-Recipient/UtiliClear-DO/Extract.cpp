...
void PC_Group_New(string DB_path, map<int, vector<string>> &group_Sign, map<int, vector<string>> &group_InSign, string NumGroup_str, int Num_InsignBits, int Num_row, int Num_col, string lamda_str,vector<vector<int>>& Grouped) {
	...
	for (int i = 1; i <= Num_row; i++) {
		...
		for (int j = 1; j <= Num_col; j++) {
			string seed_str = lamda_str + to_string(j) + to_string(i);
...
string Extract(int len, string sk, string DB_path, string FDB_path, vector<vector<int>> Grouped, int Num_group, int Num_row, int Num_col, int Num_InsignBits, int Max_ModifyBits) {
	string fingerprint;
	...
	for (int i = 1; i <= Num_row; i++) {
		...
		//cout << vec.size() << ' ' << vecF.size() << endl;
		//for (int j = 0; j < 8; j++) {
		//	cout << vec[j].size() << ' ' << vecF[j].size() << endl;
		//}
		for (int j = 1; j <= Num_col; j++) {
...
int main(int argc, const char* argv[]) {
	...
	int len = fingerprint_ori.size();
	string fingerprint = Extract(len, sk, DB_path, FDB_path, Grouped, atoi(Num_group_str.c_str()), atoi(Num_row.c_str()), atoi(Num_col.c_str()) - 1, Num_InsignBits, 18);
	int x = 0;
	for (int i = 0; i < fingerprint.size(); i++) {
		if (fingerprint[i] == fingerprint_ori[i]) {
			x++;
		}
	}
